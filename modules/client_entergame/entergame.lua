...
local forcedServerProtocol

local function ensureClientVersionOption(protocol)
    if not clientBox or not protocol then
        return
    end

    if not clientBox:isOption(protocol) then
        clientBox:addOption(protocol)
    end
...
function EnterGame.init()
    ...
    ensureClientVersionOption(clientVersion)
...
function EnterGame.doLogin()
    ...
    local clientVersion = forcedServerProtocol or tonumber(clientBox:getText())
    local httpLogin = enterGame:getChildById('httpLoginBox'):isChecked()

    if not clientVersion then
        displayErrorBox(tr('Login Error'), tr('Versão do cliente inválida ou não suportada.'))
        return
    end
...
function EnterGame.setDefaultServer(host, port, protocol)
    ...
    if hostTextEdit:getText() ~= host then
        ...
        ensureClientVersionOption(protocol)
        accountTextEdit:setText('')
        passwordTextEdit:setText('')
        authenticatorTokenTextEdit:setText('')
    end

    forcedServerProtocol = nil
end
...
function EnterGame.setUniqueServer(host, port, protocol, windowWidth, windowHeight)
    ...
    forcedServerProtocol = clientVersion
