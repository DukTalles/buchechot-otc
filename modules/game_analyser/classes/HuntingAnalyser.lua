...
-- Format large numbers with K/KK suffix for readability
-- Examples: 1500 -> "1.5K", 1500000 -> "1.5KK"
local function tokformat(value)
    if not value then return "0" end
    local numValue = tonumber(value)
    if not numValue or numValue == 0 then return "0" end
    
    local absValue = math.abs(numValue)
    local prefix = numValue < 0 and "-" or ""
    
    if absValue >= 1000000000 then
        return prefix .. string.format("%.1fB", absValue / 1000000000)
    elseif absValue >= 1000000 then
        return prefix .. string.format("%.1fKK", absValue / 1000000)
    elseif absValue >= 1000 then
        return prefix .. string.format("%.1fK", absValue / 1000)
    else
...
function HuntingAnalyser:updateWindow(ignoreVisible)
	...
	if not contentsPanel.xpGain.lastExperience or contentsPanel.xpGain.lastExperience ~= experience then
		if experience > 1000000 then
			contentsPanel.xpGain:setText(tokformat(experience))
		else
	...
	-- expSpeed is XP per SECOND, multiply by 3600 to get XP per HOUR
	HuntingAnalyser.xpHour = math.floor((player.expSpeed or 0) * 3600)
	...
	if not contentsPanel.xpHour.lastValue or contentsPanel.xpHour.lastValue ~= HuntingAnalyser.xpHour then
		if HuntingAnalyser.xpHour > 1000000 then
			contentsPanel.xpHour:setText(tokformat(HuntingAnalyser.xpHour))
		else
	...
	-- expSpeed is XP per SECOND, multiply by 3600 to get XP per HOUR
	HuntingAnalyser.rawXpHour = math.floor((player.expSpeed or 0) * 3600) --raw

	if not contentsPanel.rawXpGain.lastValue or contentsPanel.rawXpGain.lastValue ~= rawExperience then
		if rawExperience > 1000000 then
			contentsPanel.rawXpGain:setText(tokformat(rawExperience))
		else
	...
	if not contentsPanel.rawXpHour.lastValue or contentsPanel.rawXpHour.lastValue ~= HuntingAnalyser.rawXpHour then
		if HuntingAnalyser.rawXpHour > 1000000 then
			contentsPanel.rawXpHour:setText(tokformat(HuntingAnalyser.rawXpHour))
		else
	...
	if not contentsPanel.loot.lastValue or contentsPanel.loot.lastValue ~= HuntingAnalyser.loot then
		if HuntingAnalyser.loot > 1000000 then
			contentsPanel.loot:setText(tokformat(HuntingAnalyser.loot))
		else
	...
	if not contentsPanel.supplies.lastValue or contentsPanel.supplies.lastValue ~= HuntingAnalyser.supplies then
		if HuntingAnalyser.supplies > 1000000 then
			contentsPanel.supplies:setText(tokformat(HuntingAnalyser.supplies))
		else
	...
	if not contentsPanel.balance.lastValue or contentsPanel.balance.lastValue ~= HuntingAnalyser.balance then
		if math.abs(HuntingAnalyser.balance) > 1000000 then
			contentsPanel.balance:setText(tokformat(HuntingAnalyser.balance))
		else
	...
	contentsPanel.balance:setColor(HuntingAnalyser.balance >= 0 and "green" or "orange")
	...
	if not contentsPanel.damage.lastValue or contentsPanel.damage.lastValue ~= HuntingAnalyser.damage then
		if HuntingAnalyser.damage > 1000000 then
			contentsPanel.damage:setText(tokformat(HuntingAnalyser.damage))
		else
	...
	if not contentsPanel.damageHour.lastValue or contentsPanel.damageHour.lastValue ~= currentDamagePerHour then
		...
		if HuntingAnalyser.damageHour > 1000000 then
			contentsPanel.damageHour:setText(tokformat(HuntingAnalyser.damageHour))
		else
	...
	if not contentsPanel.healing.lastValue or contentsPanel.healing.lastValue ~= HuntingAnalyser.healing then
		if HuntingAnalyser.healing > 1000000 then
			contentsPanel.healing:setText(tokformat(HuntingAnalyser.healing))
		else
	...
	if not contentsPanel.healHour.lastValue or contentsPanel.healHour.lastValue ~= HuntingAnalyser.healingHour then
		if HuntingAnalyser.healingHour > 1000000 then
			contentsPanel.healHour:setText(tokformat(HuntingAnalyser.healingHour))
		else
...
function HuntingAnalyser:addLootedItems(item, name)
	...
	if not data then
		-- Use getSalePrice for loot (what NPC pays to player)
		local price = Analyser:getSalePrice(nil, itemId)
...
function HuntingAnalyser:addSuppliesItems(itemId)
	...
	if not HuntingAnalyser.suppliesItems[itemId] then
		...
		HuntingAnalyser.suppliesItems[itemId] = { count = 0, price = Analyser:getPrice(itemPtr, itemId) }
