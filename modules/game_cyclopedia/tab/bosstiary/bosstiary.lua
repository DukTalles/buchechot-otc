...
function showBosstiary()
    g_logger.info("[Bosstiary] Abrindo aba do bosstiary...")
    UI = g_ui.loadUI("bosstiary", contentContainer)
    UI:show()
    g_logger.info("[Bosstiary] Enviando requestBosstiaryInfo ao servidor...")
...
function Cyclopedia.CreateBosstiaryCreature(data)
    ...
    if not config then
        g_logger.warning("[Bosstiary] Categoria invalida: " .. tostring(data.category) .. " para raceId=" .. tostring(data.raceId))
        return
    end
    ...
    local success, err = pcall(function()
        ...
        widget.ProgressBorder2:setTooltip(
            string.format(" %d / %d %s", data.kills, config.EXPERTISE, fullText))
        widget.ProgressBorder3:setTooltip(string.format(" %d / %d %s", data.kills, config.MASTERY, fullText))

        if data.kills >= config.PROWESS then
        ...
        if data.kills >= config.EXPERTISE then
        ...
        widget.TypeIcon:setImageSource(icons[data.category] or "/game_cyclopedia/images/boss/icon_bane")
        ...
        Cyclopedia.SetBestiaryProgress(46, widget.ProgressBack, widget.ProgressBack33, widget.ProgressBack55, data.kills, config.PROWESS, config.EXPERTISE, config.MASTERY)
    ...
    if not success then
        g_logger.warning("[Bosstiary] Erro ao criar boss raceId=" .. tostring(data.raceId) .. ": " .. tostring(err))
    end
end

function Cyclopedia.LoadBosstiaryCreatures(data)
    g_logger.info("[Bosstiary] LoadBosstiaryCreatures chamado!")
    if not UI then
        g_logger.warning("[Bosstiary] UI é nil, abortando.")
        return
    end
    if not data then
        g_logger.warning("[Bosstiary] data é nil, abortando.")
        return
    end
    g_logger.info("[Bosstiary] Recebidos " .. tostring(#data) .. " bosses do servidor.")
    ...
    for i, dataEntry in ipairs(data) do
        ...
        if i <= 5 then
            g_logger.info("[Bosstiary]   Boss #" .. i .. ": raceId=" .. tostring(dataEntry.raceId) .. " cat=" .. tostring(dataEntry.category) .. " kills=" .. tostring(dataEntry.kills) .. " name=" .. tostring(raceData and raceData.name or "?"))
        end
        local creature = {
            visible = true,
    ...
    g_logger.info("[Bosstiary] Total de criaturas validas: " .. tostring(#validCreatures) .. ", TotalPages: " .. tostring(Cyclopedia.Bosstiary.TotalPages))

    Cyclopedia.LoadBosstiaryCreature(Cyclopedia.Bosstiary.Page)
    Cyclopedia.verifyBosstiaryButtons()
    g_logger.info("[Bosstiary] Carregamento concluido.")
end

function Cyclopedia.LoadBosstiaryCreature(page)
    g_logger.info("[Bosstiary] LoadBosstiaryCreature page=" .. tostring(page))
    if not Cyclopedia.Bosstiary.Creatures[page] then
        g_logger.warning("[Bosstiary] Nenhuma criatura na pagina " .. tostring(page))
        return
    end
...
function Cyclopedia.changeBosstiaryPage(prev, next)
    if not Cyclopedia.Bosstiary.Page then
        Cyclopedia.Bosstiary.Page = 1
    end
    if not Cyclopedia.Bosstiary.TotalPages then
        Cyclopedia.Bosstiary.TotalPages = 1
    end

    if next and Cyclopedia.Bosstiary.Page < Cyclopedia.Bosstiary.TotalPages then
        Cyclopedia.Bosstiary.Page = Cyclopedia.Bosstiary.Page + 1
    end

    if prev and Cyclopedia.Bosstiary.Page > 1 then
        Cyclopedia.Bosstiary.Page = Cyclopedia.Bosstiary.Page - 1
    end
