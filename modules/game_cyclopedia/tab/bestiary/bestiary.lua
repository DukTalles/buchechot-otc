...
function Cyclopedia.loadBestiaryOverview(name, creatures, animusMasteryPoints)
    g_logger.info("[Bestiary] loadBestiaryOverview chamado! name='" .. tostring(name) .. "' creatures=" .. tostring(creatures and #creatures or 'nil') .. " animusMastery=" .. tostring(animusMasteryPoints))
    if name == "Result" or name == "Search" then
        g_logger.info("[Bestiary] Carregando resultados de pesquisa...")
        Cyclopedia.loadBestiarySearchCreatures(creatures)
    else
        g_logger.info("[Bestiary] Carregando criaturas da categoria: " .. tostring(name))
...
function Cyclopedia.loadBestiarySelectedCreature(data)
    -- Sprawdź czy UI jest zainicjalizowane
    if not UI then
        return
    end
...
function Cyclopedia.loadBestiarySearchCreatures(data)
    g_logger.info("[Bestiary] loadBestiarySearchCreatures chamado! data=" .. tostring(data and #data or 'nil'))
    UI.ListBase.CategoryList:setVisible(false)
    UI.ListBase.CreatureInfo:setVisible(false)
    UI.ListBase.CreatureList:setVisible(true)
    UI.BackPageButton:setEnabled(true)
    ...
    for i = 1, #data do
        ...
        local creature = {
            ...
            AnimusMasteryBonus = data[i].AnimusMasteryBonus or data[i].creatureAnimusMasteryBonus or 0,
        }
...
function Cyclopedia.loadBestiaryCreatures(data)
    ...
    for i = 1, #data do
        ...
        local creature = {
            id = data[i].id,
            currentLevel = data[i].currentLevel,
            AnimusMasteryBonus = data[i].creatureAnimusMasteryBonus or data[i].AnimusMasteryBonus or 0,
        }
...
function Cyclopedia.BestiarySearch()
    ...
    if not text or text == "" then
        g_logger.warning("[Bestiary] Pesquisa vazia, ignorando.")
        return
    end
    g_logger.info("[Bestiary] Pesquisando por: '" .. text .. "'")
    local raceList = g_things.getRacesByName(text)
    g_logger.info("[Bestiary] Resultados encontrados: " .. tostring(#raceList))
    if #raceList > 0 then
        -- Coletar todos os raceIds para enviar ao servidor
        local raceIds = {}
        for i, race in ipairs(raceList) do
            table.insert(raceIds, race.raceId)
            if i <= 5 then
                g_logger.info("[Bestiary]   #" .. i .. ": raceId=" .. tostring(race.raceId) .. " name=" .. tostring(race.name))
            end
        end
        -- Enviar todos os IDs via requestBestiarySearchByIds (formato correto)
        if g_game.requestBestiarySearchByIds then
            g_game.requestBestiarySearchByIds(raceIds)
            g_logger.info("[Bestiary] Enviado requestBestiarySearchByIds com " .. tostring(#raceIds) .. " IDs")
        else
            -- Fallback: enviar só o primeiro (precisa recompilar)
            g_game.requestBestiarySearch(raceIds[1])
            g_logger.info("[Bestiary] Enviado requestBestiarySearch para raceId=" .. tostring(raceIds[1]))
        end
    else
        g_logger.warning("[Bestiary] Nenhuma criatura encontrada para: '" .. text .. "'. Verifique se o cliente foi recompilado com a pesquisa case-insensitive.")
    end
...
function Cyclopedia.CreateBestiaryCreaturesItem(data)
    ...
    local animusBonus = data.AnimusMasteryBonus or 0
    if animusBonus > 0 then
        widget.AnimusMastery:setTooltip("The Animus Mastery for this creature is unlocked.\nIt yields ".. animusBonus.. "% bonus experience points, plus an additional 0.1% for every 10 Animus Masteries unlocked, up to a maximum of 4%.\nYou currently benefit from ".. animusBonus.. "% bonus experience points due to having unlocked ".. (animusMasteryPoints or 0).." Animus Masteries.")
