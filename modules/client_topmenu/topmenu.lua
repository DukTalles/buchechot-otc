...
function online()
    ...
    addEvent(function()
        ...
        if not PingWidget or (PingWidget and PingWidget:isDestroyed()) then
        ...
        -- Top menu ping label depends on both showPing and feature availability
        ...
        -- Overlay panels depend only on the user's showPing preference
        if pingPanel then
            pingPanel:setVisible(showPing)
            pingImg:setVisible(showPing)
        end
        
        -- FPS overlay is always visible
        if fpsPanel2 then
...
function setFpsVisible(enable)
    ...
    -- Overlay FPS is always visible regardless of the option
    if fpsPanel2 then
        fpsPanel2:setVisible(true)
    end
end
