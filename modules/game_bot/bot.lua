...
local menuSections = {
  ...
  { id = "targeting", name = "Targeting", icon = "/images/btcbot/targeting.png" },
...
function showSection(sectionId)
  ...
  local sectionsWithToggle = {
    ...
    targeting = { module = "BTCTargeting", configKey = "targetingEnabled" },
  ...
  if sectionToggleBtn then
    ...
    if sectionInfo then
      ...
      elseif sectionId == "targeting" and BTCTargeting and BTCTargeting.config then
        isEnabled = BTCTargeting.config.enabled or false
      ...
      sectionToggleBtn.onClick = function()
        ...
        if sectionId == "cavebot" and BTCCaveBot and BTCCaveBot.config then
          ...
          print("[CaveBot] " .. (newState and "ENABLED" or "DISABLED"))
        elseif sectionId == "healing" and BTCHealing and BTCHealing.config then
        ...
        elseif sectionId == "targeting" and BTCTargeting and BTCTargeting.config then
          BTCTargeting.config.enabled = not BTCTargeting.config.enabled
          newState = BTCTargeting.config.enabled
  ...
  elseif sectionId == "targeting" then
    createTargetingUI(contentPanel)
...
function createTargetingUI(parent)
  -- Use BTC Targeting module to create UI
  if BTCTargeting and BTCTargeting.createUI then
    BTCTargeting.createUI(parent)
  else
    local label = g_ui.createWidget('Label', parent)
    label:setText('Modulo de Targeting nao carregado')
    label:setColor('#ff4444')
  end
end
