...
function BTCTargeting.execute()
  ...
  if not BTCTargeting.config or not BTCTargeting.config.enabled then return end
...
function BTCTargeting.createUI(container)
  ...
  infoLabel:setFont("verdana-9px-rounded")
end

-- Retorna status do modulo
function BTCTargeting.getStatus()
  if not BTCTargeting.config or not BTCTargeting.config.enabled then
    return "OFF"
  end
  local modes = {
    stand = "Parado",
